SELECT Razao_Social, `DATA`, REG_ANS, DESCRICAO, VL_SALDO_INICIAL, VL_SALDO_FINAL, GASTO, RANKING
FROM(
    ...
    FROM (
        ...
        SELECT
            ...
            -- CONDICIONAL PARA QUE O VALOR DE GASTO SEJA SOMADO CORRETAMENTE CONSIDERANDO CADA SEMESTRE ISOLADAMENTE
            -- POIS, HÁ SEMESTRES EM QUE O SALDO INICIAL É DIFERENTE DO SALDO FINAL DO SEMESTRE PASSADO, LOGO NÃO DEVE SER 
            -- COMPARADO COM OS VALORES DO SEU ANTERIOR, DESTA FORMA HÁ APENAS A SOMA TOTAL DE GASTOS EM SI DESCONSIDERANDO ACRESCIMOS EXTERNOS
            SUM(
                CASE 
            ...
            FORMAT(
                SUM(
                    CASE 
                        ...
                        ELSE 0 
                    END
                ) OVER (PARTITION BY REG_ANS), 2) AS GASTO -- FORMATADO PARA MELHOR LEGIBILIDADE
        ...
        WHERE DESCRICAO LIKE '%a saude medico hospitalar%' AND `DATA` >= "2024-10-01"
